...
ProcessHubWebhookJob::handle() calls bindAccountContext() first, then reads WebhookCall via resolveWebhookCall() and alreadyProcessed(). Both use the default connection *after* the switch.
